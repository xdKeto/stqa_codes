...
def fill_pet_form(context, name="", category="", gender=None, birthday=None, available=True):
    ...
    if birthday:
        ...
        date_element.send_keys(birthday)
        context.driver.execute_script("arguments[0].value = arguments[1];", date_element, birthday)
...
def step_impl(context, pet_id, pet_name):
    ...
    # Workaround: app.py wipes missing fields, so we must re-fill them
    context.driver.find_element(By.ID, "pet_category").send_keys("dog")
    date_element = context.driver.find_element(By.ID, "pet_birthday")
    context.driver.execute_script("arguments[0].value = arguments[1];", date_element, "2025-01-01")
